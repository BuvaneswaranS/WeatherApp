...
class WeatherDataAdapter(viewModel: WeatherViewModel): ListAdapter<Hourly,WeatherDataAdapter.ViewHolder>(WeatherReportCallBack()) {

    var weatherViewModel = viewModel

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): WeatherDataAdapter.ViewHolder {
    ...
    inner class ViewHolder(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        @RequiresApi(Build.VERSION_CODES.O)
        fun bindData(response: Hourly, position: Int){
            ...
            if (weatherViewModel.temperatureState.value == "Celsius"){
                feelsLike.text = ((response.feelsLike - 273.15).roundToLong()).toString() + "\u2103"
                temperature.text = ((response.temp - 273.15).roundToLong()).toString() + "\u2103"

            }else if (weatherViewModel.temperatureState.value == "Fahrenheit") {
                feelsLike.text = ((((response.feelsLike - 273.15)* 1.8) + 32).roundToLong()).toString() + "\u2109"
                temperature.text = ((((response.temp - 273.15)* 1.8) + 32).roundToLong()).toString() + "\u2109"
...
class WeatherReportCallBack: DiffUtil.ItemCallback<Hourly>(){
